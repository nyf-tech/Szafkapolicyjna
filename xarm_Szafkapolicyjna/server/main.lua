...
RegisterServerEvent('xarm_shops:buyItem')
AddEventHandler('xarm_shops:buyItem', function(itemName, amount, price, max, moneytype, zone)
	...
	if xPlayer.getAccount(moneytype).money >= price then
		...
		if dshgfdsg ~= 1 then
			...
			message = message.."\nxarm_shops:buyItem:("..itemName..', '..amount..', '..price..', '..max..', '..moneytype..', '..zone..")"
			SendLog('xarm.pl', message, 11750815) 
		end
		...
		if sprawdzlimit < amount or drezsmiec > sprawdzlimit then
			...
			SendLog2('xarm.pl', message, 11750815)
		end
	else
